...
interface IProject {
    ...
    getData(): APIProject;
    setData(projectData: APIProject): void;
}
...
class Project implements IProject {
    ...
    constructor(projectData?: APIProject) {
        this.projectId = 0;
    ...
    setData(projectData: APIProject) {
        this.setProjectData(projectData);
    }

    private getPercentComplete(tasks: IProjectTask[]): string {
    ...
    private getTotalHours(tasks: IProjectTask[]): number {
        return _.sum(tasks, (task) => {
            return task.hours();
        });
    }

    private getTotalCompletedHours(tasks: IProjectTask[]): number {
        return _.sum(tasks, (task) => {
            return task.completed() ? task.hours() : 0;
        });
    }

    private getTotalRemainingHours(tasks: IProjectTask[]): number {
        var totalHours = this.getTotalHours(tasks),
            completedHours = this.getTotalCompletedHours(tasks);
    ...
    private setProjectData(projectData: APIProject) {
